    ...
    for index, cord in enumerate(image_cord_entry):
        ...
        if eq_entry != "":
            ...
            if flag and len(values) == 6:
                eq_cord = (int(values[0]), int(values[1]), int(values[2]), int(values[3]), int(values[4]), int(values[5]))
            else:
    ...
    if autofill_path != "":
        if exists(autofill_path) and autofill_path[-3:] == "txt":
            ...
            for index in range(8, len(data)):
                ...
                eq_cord = (int(eq_cord[0]), int(eq_cord[1]), int(eq_cord[2]), int(eq_cord[3]), int(eq_cord[4]), int(eq_cord[5]))
            ...
            if len(star_eq_cord_temp) > 4:
                ...
                print(star_img_cord_temp)
                print(star_eq_cord_temp)
...
def comd_align():
    ...
    general_info = [file_path, position, current_date, current_time, arcsec, zenith_ap, offset, height]
    ...
    zenith_pos = zenith_1.zenith_alignment()
    north_vector = zenith_1.get_north_vector()
    north_offset = 360 - cal_angle(north_vector)
    ...
    north_out_text.set(f'{north_offset}')
    zenith_out_text.set(f'{zenith_pos[0]}, {zenith_pos[1]}')
    ...
    for pos in star_img_cord:
        ...
        cv2.line(blank, (int(pos[0]), int(pos[1])), zenith_pos, (0, 255, 0), thickness=2)
    ...
    blank_resize = rescale_frame(blank, 0.19)
...
label_names = ["File Path : ", "Position : ", "Date : ", "Time : ",
               "Arcsec_px : ", "Zenith AP : ", "North off AP : ", "Height : ", "Autofill file : "]
general_entry = []
...
if next_sep_flag:
    ...
    def translate_coordinates(star_pos, image_zenith, north_offset):
        ...
        zenith_star_pos = rotate_point(360 - north_offset, zenith_star_vector_off) + image_zenith
    ...
    label_ = Label(window, text=f'{zenith_pos[0]}')
    label_.configure(bg='#404040', fg='white')
    ...
    label_ = Label(window, text=f'{zenith_pos[1]}')
    label_.configure(bg='#404040', fg='white')
    ...
    label_ = Label(window, text=f'{north_offset}')
    label_.configure(bg='#404040', fg='white')
    ...
    def comd_alignment(distortion_adj=False):
        ...
        zenith_pos_adj = zenith_pos[0] + zenith_x_adj, zenith_pos[1] + zenith_y_adj
        north_offset_adj = north_offset + offset_adj
        ...
        for index, manual_star in enumerate(star_objects):
            ...
            if distortion_adj is True and distortion_flag is True:
                distortion = (distortion_parameters[0] * dist) + (distortion_parameters[1] * (dist ** 2)) + (distortion_parameters[2] * (dist ** 3)) + (distortion_parameters[3])
            else:
                distortion = 0
        ...
        pyplot.title(label="Distortion Values")
        pyplot.xlabel('Distance to image center (px)')
        pyplot.ylabel('Distortion (px)')

        blank_r = rescale_frame(blank, 0.15)
